...
local SAVE_AS_JSON = false
local SAVE_AS_CSV = false
...
local bestDay = 0
local bestDayDate = {week=0,day=0} 
local bestWeek = 0
local bestMonth = 0
local bestYear = 0
...
buttons = {
    ...
    ['animations'] = {x=100,y=windowHeight - 19,w=16,h=16},
    ['studentPics'] = {x=210,y=windowHeight - 19,w=16,h=16},
    ['showEvents'] = {x=320,y=windowHeight - 19,w=16,h=16},
    ['saveON'] = {x=482,y=windowHeight - 19,w=16,h=16},
    ['saveJSON'] = {x=592,y=windowHeight - 19,w=16,h=16},
    ['saveCSV'] = {x=702,y=windowHeight - 19,w=16,h=16},
...
function writeLine(day,class)
    ...
    local line =    'addClass{year='..week[day][class].year.. -- 23/8/23 added a '()' around the '{}' so data can be used by javascript
    ...
    line = line..'}' -- 23/8/23 added a '()' around the '{}' so data can be used by javascript
    return line
end

-- constructs each line to be written to classData.json --! I think it's done
-- called from writeWeek
function writeLineJSON(day,class)
    --print('student '..week[day][class].name)
    --print('has num ' ..getStudentNum(week[day][class].name))

    local line =    '\t{\n\t\t"year":'..week[day][class].year..',\n'..
                    '\t\t"month":'..week[day][class].month..',\n'..
                    '\t\t"date":'..week[day][class].date..',\n'..
                    '\t\t"hour":'..week[day][class].hour..',\n'..
                    '\t\t"min":'..week[day][class].min..',\n'..
                    ...
                    '\t\t"paid":'..week[day][class].paid..''
    if week[day][class].class then
        line = line ..  ',\n\t\t"class":'..week[day][class].class..''
        if week[day][class].pkg then
            line = line .. ',\n\t\t"pkg":'..week[day][class].pkg..''
        end
    end
    if week[day][class].wasClass1 then
        line = line .. ',\n\t\t"wasClass1":true'
    ...
    if week[day][class].note then
        line = line ..  ',\n\t\t"note":"'..week[day][class].note..'"'
    end
    line = line..'\n}'
    return line
end

-- constructs each line to be written to classData.csv --! I think it's done
-- called from writeWeekCSV
function writeLineCSV(day,class)
    --print('student '..week[day][class].name)
    --print('has num ' ..getStudentNum(week[day][class].name))

    local line =    '"'..week[day][class].year..'",'..
                    '"'..week[day][class].month..'",'..
                    '"'..week[day][class].date..'",'..
                    '"'..week[day][class].hour..'",'..
                    '"'..week[day][class].min..'",'..
                    '"'..week[day][class].slot..'",'..
                    '"'..week[day][class].length..'",'..
                    '"'..week[day][class].num..'",'..
                    '"'..week[day][class].name..'",'..
                    '"'..week[day][class].status..'",'..
                    '"'..week[day][class].paid..'"'
    if week[day][class].class then
        line = line ..  ',"'..week[day][class].class..'"'
        if week[day][class].pkg then
            line = line .. ',"'..week[day][class].pkg..'"'
        else
            line = line .. ',""'
        end
    else
...
-- returns the year, month, and day of the passed weekNum and dayNum
-- working here
function dateOfClass(weekNum, dayNum)
    -- local year = 0
    -- local month = 0
    -- local day = 0

    local startDate = os.date("*t",os.time{year=startYear,month=startMonth,day=startDay})
    local startutc = os.time(startDate)
    local dayAdjust = (weekNum * 7) - dayNum + 1 -- days to adjust from week 1 monday?
    local thisDate = os.date("*t",startutc + dayAdjust * 24 * 3600)
    -- print('weeknum '..weekNum..' dayNum '..dayNum)
    -- print('Year '..thisDate.year..' Month '..thisDate.month..' Day '..thisDate.day)

    return thisDate.year, thisDate.month, thisDate.day
end
...
-- which will not have any cancellations and so incorrectly calculate totals
    ...
    for i = 1,thisWeekNum do -- 11/4/22 changed from totalDataWeeks to thisWeekNum
        for j = 1,8 do
            if newWeekTotals[i] then 
                ...
                if newWeekTotals[i][j] > dayRecords[j] then 
                    dayRecords[j] = newWeekTotals[i][j] 
                    -- print('dayRecords['..j..'] is '..dayRecords[j]) 
                    if j == 8 then -- check for best week total
                        bestDayDate.week = i -- store the week num of the best day
                        -- print('best day is in week num '..i..' with '..newWeekTotals[i][j]..' classes') 
                    end
                end
            end
        end
    ...
    for k = 1, 7 do 
        if dayRecords[k] > bestDay then 
            bestDay = dayRecords[k] 
            -- print('best week num '..bestDayDate.week) 
            bestDayDate.day = k -- store the day num of the best day
        end 
    end
...
-- write all class data from active week to classData.json
-- called from writeAllClasses
function writeWeekJSON(lastWeek)
    local weekDay = D.currentDayOfWeek() - 1
...
--            printIfDebug(writeLine(day,class))
            ...
            -- ! and don't write if it is a 'new class' that has not been assigned to a student
            if week[day][class].delete ~= 1 and week[day][class].name ~= 'new class' then 
                fileToWrite:write(',\n'..writeLineJSON(day,class)) --! FUDGE to write comma at start
                --fileToWrite:write(',') --! ONLY ADD comma if more classes FUGDE remove comment
                fileToWrite:write('\n') 
            end
        end
    end
...
-- write all class data from active week to classData.json
-- called from writeAllClasses
function writeWeekCSV(lastWeek)
    local weekDay = D.currentDayOfWeek() - 1
...
--            printIfDebug(writeLine(day,class))
            ...
            -- ! and don't write if it is a 'new class' that has not been assigned to a student
            if week[day][class].delete ~= 1 and week[day][class].name ~= 'new class' then 
                fileToWrite:write(writeLineCSV(day,class)) --! FUDGE to write comma at start
                --fileToWrite:write(',') --! ONLY ADD comma if more classes FUGDE remove comment
                -- fileToWrite:write('\n') 
            end
        end
...
-- writes the classData.csv file --! this should work
-- called from writeAllData
function writeAllClassesCSV()
    print('In writeAllClassesCSV')
    -- OPEN FILE    
        fileToWrite = io.open(compDir..'classData.csv', 'w')
        
        weekOffset = getWeekOffsetFor(firstDay,firstMonth,firstYear)

        for i = weekOffset, totalDataWeeks + 1 do
            if CLOSE_DEBUG then print('Week '..i) end
            if i == totalDataWeeks + 1 then writeWeekCSV(true)
            else writeWeekCSV(false) end
            weekOffset = weekOffset + 1
        end
...
-- writes the classData.json file --! this should work
-- called from writeAllData
function writeAllClassesJSON()
    print('In writeAllClassesJSON')

    -- OPEN FILE    
        fileToWrite = io.open(compDir..'classData.json', 'w')
    ...
    --weekOffset = -1000
        local dummy = '\t{\n'..
                    ...
                    '\t\t"status":-1,\n'..
                    '\t\t"paid":0}\n'
        fileToWrite:write('[\n'..dummy)     -- ! write opening bracket FUDGE remove {}

        for i = weekOffset, totalDataWeeks + 1 do
            if CLOSE_DEBUG then print('Week '..i) end
            if i == totalDataWeeks + 1 then writeWeekJSON(true)
            else writeWeekJSON(false) end
            weekOffset = weekOffset + 1
        end
        fileToWrite:write(']')     -- ! write closing bracket
...
--        local line = 'addStudent{name=\''..student[i].name..'\''..
        local line = 'addStudent{num='..student[i].num.. -- 23/8/23 added a '()' around the '{}' so data can be used by javascript
        ',name=\''..student[i].name..'\''..
...
-- writes the studentData.csv file
-- called from writeAllData
function writeAllStudentsCSV()
    print('In writeAllStudentsCSV')
    fileToWrite = io.open(compDir..'studentData.csv', 'w')
    for i = 1, #student do
        local line = '"'..student[i].num..'",'..
        '"'..student[i].name..'",'..
        '"'..student[i].sType..'",'..
        '"'..student[i].package..'",'..
        '"('
        ...
        '"'..student[i].startMth..'",'..
        '"'..student[i].startDay..'",'..
        '"'..student[i].classesNishCancelled..'"'
        -- write birthday info
        if student[i].birthday then line = line..',"'..student[i].birthday..'"' 
        else line = line..',""' end
        -- write image info
        if student[i].studentImageFileName then line = line..',"'..student[i].studentImageFileName..'"' 
        else line = line..',""' end
        -- write phone info
        if student[i].phone then line = line..',"'..student[i].phone..'"' 
        else line = line..',""' end
        if student[i].sNote then line = line..',"'..student[i].sNote..'"' 
        else line = line..',""' end
        fileToWrite:write(line..'\n')            
    end
    io.close(fileToWrite)
...
-- writes the studentData.json file
-- called from writeAllData
function writeAllStudentsJSON()
    fileToWrite = io.open(compDir..'studentData.json', 'w')
    fileToWrite:write('[\n')            
    for i = 1, #student do
--        local line = 'addStudent{name=\''..student[i].name..'\''..
        local line = '\t{\n\t\t"num":'..student[i].num..',\n'..
        ...
        '\t\t"classesNishCancelled":'..student[i].classesNishCancelled..''
        -- write birthday info
        if student[i].birthday then line = line..',\n\t\t"birthday":"'..student[i].birthday..'"' end
        -- write image info
        if student[i].studentImageFileName then line = line..',\n\t\t"studentImageFileName":"'..student[i].studentImageFileName..'"' end
        -- write phone info
        if student[i].phone then line = line..',\n\t\t"phone":"'..student[i].phone..'"' end
        if student[i].sNote then line = line..',\n\t\t"sNote":"'..student[i].sNote..'"' end
        line = line..'}'
        if i < #student then line = line..',' end
        fileToWrite:write(line..'\n')            
    end
    fileToWrite:write(']')            
    io.close(fileToWrite)
...
if CLOSE_DEBUG then print('Classes ...') end
    writeAllClasses()
    if SAVE_AS_JSON then writeAllClassesJSON() end
    if SAVE_AS_CSV then writeAllClassesCSV() end --! still to do (make a radio button like DONT SAVE)
...
function love.wheelmoved( dx, dy)
    -- print('in wheelmoved dx = '..dx..' dy = '..dy)
    if showAllStudents then
        startStudentNumber = math.max(startStudentNumber - dy,1)
    end
    _dx = dx
    _dy = dy
...
-- adjust when scaled            
    ...
    elseif clicked(buttons.showEvents,mx,my,button) then
        showEvents = not showEvents

    -- toggle data saving
    elseif clicked(buttons.saveON,mx,my,button) then
        QUIT_WITHOUT_SAVING = not QUIT_WITHOUT_SAVING
    elseif clicked(buttons.saveJSON,mx,my,button) then
        SAVE_AS_JSON = not SAVE_AS_JSON
    elseif clicked(buttons.saveCSV,mx,my,button) then
        SAVE_AS_CSV = not SAVE_AS_CSV
...
function addNewClass()
    ...
    selectedStudentName = ''
    local weekDay = D.currentDayOfWeek() - 1
    if weekDay == 0 then weekDay = 7 end -- Windows Key + Period -> emoji picker 🌮
    local dayAdjust = (weekOffset * 7) - weekDay
...
--        if not zeroPackage(thisClass.name) then
    ...
    if mouseOver(xPos, yPos, thisClass.width, thisClassHeight) and topMost(day, xPos, yPos, thisClass) then
        ...
        if not chosenStudent and not addingNewStudent then 
            displayStudentDetails(thisClass) 
            -- if mouse is over a student's SNote then display the note HERE!
--            if mouseOver(xPos + thisClass.width - 20, yPos, 20, 20) then
                if findStudent(thisClass.name).sNote then
                    c_lgp(findStudent(thisClass.name).sNote,398,812)
                end
            --end
        end
...
function drawWeekHighlights()
    if not showEvents then return end
    ...
    if numLines > maxLines then
        ...
        -- this causes a bug with highlighting the correct student
        -- need to offset when hovering over a note
    end

    if overAnyStudent and numLines > 20 then return end
    -- if numLines > 30 then return end -- added this 24/3/2024 to not show if too big
...
function drawTotalsLabels(startLine,gap)
    ...
    c_lgp('Average Day',600,startLine + 5 * gap)
    c_lgp('Average Week',600,startLine + 6 * gap)
    c_lgp('Average Month',600,startLine + 7 * gap)
    c_lgp('Average Year',600,startLine + 8 * gap)
...
function drawAllTimeTotals()
    ...
    -- call function to return date HERE
    local yr, mth, day = dateOfClass(bestDayDate.week,bestDayDate.day)
    -- print('Year '..yr..' month '..mth..' day '..day)
    c_lgpf(day..'/'..mth..'/'..yr,470,startLine + 5 * gap, 100, 'center')
    -- c_lgpf(bestDayDate.week,470,startLine + 5 * gap, 100, 'center')
    -- c_lgpf(bestDayDate.day,490,startLine + 5 * gap, 100, 'center')
    ...
    c_lgpf(round2(totalNumClasses/totalDataDays,2),670,startLine + 5 * gap, 100, 'center')
    c_lgpf(round2(totalNumClasses/totalDataWeeks,2),670,startLine + 6 * gap, 100, 'center')
    c_lgpf(round2(totalNumClasses/numMonths,2),670,startLine + 7 * gap, 100, 'center')
    c_lgpf(round2(totalNumClasses/numYears),670,startLine + 8 * gap, 100, 'center')
...
function drawBottomLine()
    ...
    c_lgp('dx = '.._dx..' dy = '.._dy,windowWidth-250,windowHeight - 18)   

    c_lgp('Animations',buttons.animations.x - 82,windowHeight - 18) 
    c_lgc('line',buttons.animations.x + 8,windowHeight - 11,8)
    if scrolling then c_lgc('fill',buttons.animations.x + 8,windowHeight - 11,4) end

    c_lgp('Student Pics',buttons.studentPics.x - 82,windowHeight - 18) 
    c_lgc('line',buttons.studentPics.x + 8,windowHeight - 11,8)
    if showStudentPics then c_lgc('fill',buttons.studentPics.x + 8,windowHeight - 11,4) end

    c_lgp('Show Events',buttons.showEvents.x - 82,windowHeight - 18) 
    c_lgc('line',buttons.showEvents.x + 8,windowHeight - 11,8)
    if showEvents then c_lgc('fill',buttons.showEvents.x + 8,windowHeight - 11,4) end

    c_lgp(' DONT SAVE',buttons.saveON.x - 82,windowHeight - 18) 
    c_lgc('line',buttons.saveON.x + 8,windowHeight - 11,8)
    if QUIT_WITHOUT_SAVING then c_lgc('fill',buttons.saveON.x + 8,windowHeight - 11,4) end

    c_lgp('  SAVE JSON',buttons.saveJSON.x - 82,windowHeight - 18) 
    c_lgc('line',buttons.saveJSON.x + 8,windowHeight - 11,8)
    if SAVE_AS_JSON then c_lgc('fill',buttons.saveJSON.x + 8,windowHeight - 11,4) end

    c_lgp('    SAVE CSV',buttons.saveCSV.x - 82,windowHeight - 18) 
...
function packageFee(package,name)
    if name == 'Giulia' then return 46500
    elseif name == 'Barbara' then return 50000
    elseif name == 'Nicoletta' then return 36000
    elseif name == 'Ilaria' then return 14500
    elseif package == 1 then return 3000
    elseif package == 8 then return 18000
    elseif package == 12 then return 21000
    elseif package == 16 then return 23000
    elseif package == 20 then return 25000
    end
end
